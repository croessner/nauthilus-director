...
printf 'ok e2e: fake Nauthilus HTTP/gRPC authority, OIDC Bearer caller auth, insufficient-scope denial, real server binary, fake IMAP, LMTP, ManageSieve and POP3 backends, outbound backend PROXY, backend pinning, user placement holds, runtime repair, listener drain/resume, DATA/BDAT, TLS, route lookup and observability checks passed\n'
