...
- Build and packaging references use Go 1.26.8.
...
| --- | --- | --- | --- | --- |
| Production binaries | `cmd/nauthilus-director/`, `cmd/nauthilus-directorctl/`, `Makefile` | `make build-check`; `make build` for concrete binary output | In `make guardrails` through `build-check` | Binaries must build from the root production module with Go 1.26.8. |
