...
--
-- Reads one user placement hold without mutating affinity or hold state.

local hold_key = KEYS[1]

local fallback_tenant = ARGV[1]
local fallback_account_key = ARGV[2]
...
local generation = require_value(redis.call("HGET", hold_key, "generation"), "generation_required")
...
local updated_at = require_number(redis.call("HGET", hold_key, "updated_at_ms"), "updated_at_required")
...
if expires_at <= now then
	return {
		...
		"updated_at_ms", tostring(updated_at),
		"server_time_ms", tostring(now)
	}
end
