...
const (
	...
	operationRouteLookup = "route_lookup"
...
// RouteLookupService explains routing through selector and backend read services.
...
func (s *RouteLookupService) Lookup(ctx context.Context, request RouteLookupRequest) (RouteLookupResponse, error) {
	...
	if request.Protocol == "" {
		return RouteLookupResponse{}, newRuntimeError(ErrorKindInvalidRequest, operationRouteLookup, "protocol required")
	}

	if request.AccountKey == "" {
		return RouteLookupResponse{}, newRuntimeError(ErrorKindInvalidRequest, operationRouteLookup, "account key required")
	}
	...
	usedDefaultShard := request.RequestedShard == ""
	if request.RequestedShard == "" {
		request.RequestedShard = s.defaultShard
	}

	selectionRequest := backend.SelectionRequest{
		AccountKey:  request.AccountKey,
		Tenant:      request.Tenant,
		ShardTag:    request.RequestedShard,
		Protocol:    request.Protocol,
		BackendPool: request.BackendPool,
	}
	result, err := s.selector.Select(ctx, selectionRequest)

	response := s.initialLookupResponse(ctx, request, usedDefaultShard)
	...
	response.SelectedBackend = result.Backend.Identifier
	response.ReasonClass = result.Reason
	response.Routing.RoutingGeneration = result.Generation
	response.Routing.EffectiveShard = result.EffectiveBackend.EffectiveShardTag

	return response, nil
}

// initialLookupResponse builds the side-effect-free route lookup response shell.
func (s *RouteLookupService) initialLookupResponse(ctx context.Context, request RouteLookupRequest, usedDefaultShard bool) RouteLookupResponse {
	return RouteLookupResponse{
		Routing: RouteLookupRoutingState{
			AccountKey:       request.AccountKey,
			Tenant:           request.Tenant,
			RequestedShard:   request.RequestedShard,
			EffectiveShard:   request.RequestedShard,
			RoutingSource:    "operator_request",
			UsedDefaultShard: usedDefaultShard,
		},
		Backends: s.lookupBackends(ctx, request),
	}
}

// lookupBackends returns safe candidate summaries when the read service is available.
func (s *RouteLookupService) lookupBackends(ctx context.Context, request RouteLookupRequest) []RouteLookupBackendState {
	...
	for _, state := range states {
		...
		if request.RequestedShard != "" && state.EffectiveShardTag != request.RequestedShard {
			continue
		}

		backends = append(backends, NewRouteLookupBackendState(state))
	}
