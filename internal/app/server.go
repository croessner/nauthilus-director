...
func provideControlHandle(
	...
	runtimeAggregateService := runtimectl.NewRuntimeAggregateService(store, runtimectl.WithObservabilityRecorder(recorder))
	...
	handler := rest.NewServer(rest.Options{
		...
		HandlerOptions: adapters.HandlerOptions{
			...
			UserHoldReader:             userHolds,
			RuntimeSummaryReader:       runtimeReader,
			RuntimeAggregateReconciler: runtimeAggregateService,
...
func imapSessionHandler(
	...
	return imap.NewHandler(imap.SessionConfig{
		...
		CertificateAuthenticator:     options.CertificateAuthenticator,
		AllowExternalAuthorizationID: options.ExternalAuthPolicy.AllowAuthorizationID,
