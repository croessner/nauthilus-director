...
| E2E release | Clear releases waiting placement to pinned target in the selected shard and compatible backend node | The test sets `users backend-pin` to a same-shard/backend-node target, clears the hold through CLI and verifies the waiting login proxies to the pinned backend | OK | Demo stack repeats this with `prove-user-hold.sh` |
